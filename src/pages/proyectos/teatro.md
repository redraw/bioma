---
layout: ../../layouts/ProyectoLayout.astro
title: Teatro
category: c u l t u r a l
description: La restauración del teatro ópera de La Plata llevó un estudio
  minucioso de lo existente, respecto a sus intervenciones posteriores, y a su
  potencial como sede de la congregación y cultura musical platense. Pensándolo
  como un centro cultural, el proyecto se basa en la actualización de la
  infraestructura urbana que significa un teatro clásico.
created: April 21, 2024 7:37 PM
draft: false
cover: https://res.cloudinary.com/biomaarquitectos/image/upload/c_scale,f_auto,q_auto,w_160/v1713739255/TEATRO/2.OPERA_Render_Frente-min_ytntku.jpg
logo: https://res.cloudinary.com/biomaarquitectos/image/upload/c_scale,f_auto,q_auto,w_160/v1713035319/11teatro_k6fyah.png
mostrar_home_grid: true
mostrar_home_sliders: false
slider_fotos:
  autoplay: false
  effect: fade
slider_planos:
  autoplay: false
  effect: fade
blocks:
  - type: grid
    ratio: "1"
    align: center
    gap: 4
    columns:
      - images:
          - https://res.cloudinary.com/biomaarquitectos/image/upload/c_scale,f_auto,q_auto,w_160/v1713739255/TEATRO/4.OPERA_Render_Interior-min_artovf.jpg
  - type: masonry
    columns: 2
    gap: 20
    images:
      - https://res.cloudinary.com/biomaarquitectos/image/upload/c_scale,f_auto,q_auto,w_160/v1713753424/TEATRO/localhost_4321_proyectos_teatro-min_iqsdkp.png
      - https://res.cloudinary.com/biomaarquitectos/image/upload/c_scale,f_auto,q_auto,w_160/v1713753424/TEATRO/localhost_4321_proyectos_teatro_1_-min_rqhpve.png
---
