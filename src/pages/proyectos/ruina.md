---
layout: ../../layouts/ProyectoLayout.astro
title: Ruina
category: vivienda
description: >-
  Para proyectar la casa partimos de la visión intrigante de una ruina
  envolvente que consolida su perímetro.\

  Por dentro descubrimos un oasis que contrasta con la decadencia exterior, la selva que ha tomado la ruina se convierte en una parte integral del diseño, fusionando la naturaleza con la arquitectura permitiendo la convivencia entre lo antiguo y lo vivo, fusionando la historia y la renovación.
created: April 15, 2024 2:42 AM
draft: false
cover: https://res.cloudinary.com/biomaarquitectos/image/upload/c_scale,f_auto,q_auto,w_160/v1713711561/RUINA/1.RUINA_Exterior-min_afm73i.jpg
logo: https://res.cloudinary.com/biomaarquitectos/image/upload/c_scale,f_auto,q_auto,w_160/v1713035315/07ruina_qesff5.png
mostrar_home_sliders: false
slider_fotos:
  autoplay: false
  effect: fade
slider_planos:
  autoplay: false
  effect: fade
blocks:
  - type: grid
    ratio: 2/1
    align: stretch
    gap: 4
    columns:
      - images:
          - https://res.cloudinary.com/biomaarquitectos/image/upload/c_scale,f_auto,q_auto,w_160/v1713711559/RUINA/2.RUINA_Maqueta_I-min_t0b5ld.jpg
      - images:
          - https://res.cloudinary.com/biomaarquitectos/image/upload/c_scale,f_auto,q_auto,w_160/v1713711559/RUINA/3.RUINA_Maqueta_II-min_ezlvrg.jpg
  - type: text
    content: La maqueta como proceso, un tablero de juego conformado por una grilla
      de puntos permitió que la ideación del proyecto fuera dado en su totalidad
      mediante las pruebas, el error como motor y la sintesis como norte. La
      espacialidad, iluminación, estructura y material sugeridos e involucrados
      en el proceso.
  - type: masonry
    columns: 1
    gap: 4
    images:
      - https://res.cloudinary.com/biomaarquitectos/image/upload/c_scale,f_auto,q_auto,w_160/v1713711563/RUINA/4.RUINA_Planta-min_oyjv3s.jpg
  - type: masonry
    columns: 2
    gap: 4
    images:
      - https://res.cloudinary.com/biomaarquitectos/image/upload/c_scale,f_auto,q_auto,w_160/v1713711560/RUINA/7.RUINA_Corte-min_qowor6.jpg
      - https://res.cloudinary.com/biomaarquitectos/image/upload/c_scale,f_auto,q_auto,w_160/v1713711563/RUINA/4.RUINA_Planta-min_oyjv3s.jpg
      - https://res.cloudinary.com/biomaarquitectos/image/upload/c_scale,f_auto,q_auto,w_160/v1713711560/RUINA/8.RUINA_Interior-min_uhllw1.jpg
      - https://res.cloudinary.com/biomaarquitectos/image/upload/c_scale,f_auto,q_auto,w_160/v1713711560/RUINA/9.RUINA_Patio-min_drritt.jpg
---
